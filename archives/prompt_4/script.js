// Fonction pour ouvrir le LLM choisi avec le prompt
function openInLLM(prompt) {
  const llmSelector = document.getElementById('llm-selector');
  const selectedLLM = llmSelector.value;
  
  if (!prompt || prompt.trim() === '') {
    alert('Aucun prompt à ouvrir !');
    return;
  }
  ...
  if (selectedLLM === 'chatgpt') {
    // ChatGPT : ouvre avec le prompt pré-rempli
    url = `https://chat.openai.com/?q=${encodedPrompt}`;
    window.open(url, '_blank');
  } else if (selectedLLM === 'claude') {
    // Claude : copie dans le presse-papier et ouvre claude.ai
    url = 'https://claude.ai/chat';
    navigator.clipboard.writeText(prompt).then(() => {
      window.open(url, '_blank');
      // Petite notification pour indiquer que c'est copié
      setTimeout(() => {
        alert('✅ Prompt copié dans le presse-papier ! Collez-le dans Claude et envoyez.');
      }, 500);
    }).catch(() => {
      window.open(url, '_blank');
      alert('⚠️ Ouvrez Claude et copiez manuellement le prompt affiché ci-dessous.');
    });
  } else if (selectedLLM === 'grok') {
    // Grok : copie dans le presse-papier et ouvre grok
    url = 'https://x.com/i/grok';
    navigator.clipboard.writeText(prompt).then(() => {
      window.open(url, '_blank');
      setTimeout(() => {
        alert('✅ Prompt copié dans le presse-papier ! Collez-le dans Grok et envoyez.');
      }, 500);
    }).catch(() => {
      window.open(url, '_blank');
      alert('⚠️ Ouvrez Grok et copiez manuellement le prompt affiché ci-dessous.');
    });
  }
}
...
Produis directement le contenu demandé, prêt à être utilisé.
  ...
  openInLLM(simplePrompt);
}

// Génération du prompt VS (Verbalized Sampling) pour la rédaction
function generateVSPrompt() {
  const type = getFieldValue("redac-type-select", "redac-type-custom");
  const objectif = getFieldValue(
    "redac-objectif-select",
    "redac-objectif-custom"
...
${detailDemande}
  ...
  const vsInstructions = `<mission_verbalized_sampling>

<persona_expert>
Tu es un expert créatif en génération de contenu avec une maîtrise avancée du Verbalized Sampling. Tu excelles dans l'exploration de multiples angles créatifs pour produire des variations innovantes et pertinentes.
</persona_expert>

<methodologie_verbalized_sampling>
Le Verbalized Sampling est une technique de créativité qui génère plusieurs variations d'un même contenu en explorant différentes perspectives, tonalités et approches. Chaque variation apporte une dimension unique tout en respectant les contraintes de base.

<etape numero="1">
ANALYSE : Identifie les éléments clés de la demande et les axes de variation possibles (ton, angle, structure, style, niveau de détail).
</etape>

<etape numero="2">
GÉNÉRATION : Crée 5 variations créatives distinctes :
  • Variation 1 : APPROCHE CLASSIQUE - Version professionnelle et structurée
  • Variation 2 : APPROCHE NARRATIVE - Utilise le storytelling et l'émotion
  • Variation 3 : APPROCHE ANALYTIQUE - Focus sur les données et la logique
  • Variation 4 : APPROCHE CONVERSATIONNELLE - Ton proche et engageant
  • Variation 5 : APPROCHE INNOVANTE - Perspective originale et inattendue
</etape>

<etape numero="3">
PRÉSENTATION : Pour chaque variation, fournis :
  - Le contenu complet prêt à utiliser
  - L'approche créative utilisée
  - Les points forts de cette variation
</etape>

<etape numero="4">
RECOMMANDATION : À la fin, identifie quelle variation est la plus adaptée selon le contexte et pourquoi.
</etape>
</methodologie_verbalized_sampling>
...
## 🎨 VARIATION [N]/5 : [NOM APPROCHE]
### 📝 Contenu :
\`\`\`
[Contenu complet prêt à utiliser]
\`\`\`
...
### 🎯 Meilleure variation : [N]
### 💬 Justification :
[Explication détaillée du choix en fonction du contexte, du public cible et des objectifs]

### 🔄 Option hybride :
[Suggestion de combinaison des meilleures idées de plusieurs variations si pertinent]
</format_reponse>
...
</mission_verbalized_sampling>`;

  document.getElementById("result").textContent = vsInstructions;
  openInLLM(vsInstructions);
}
...
// Génération du prompt VS pour les images
function generateVSImagePrompt() {
  const type = getFieldValue("img-type-select", "img-type-custom");
  const style = getFieldValue("img-style-select", "img-style-custom");
  const couleurs = getFieldValue("img-couleurs-select", "img-couleurs-custom");
  const eclairage = getFieldValue("img-eclairage-select", "img-eclairage-custom");
  const ambiance = getFieldValue("img-ambiance-select", "img-ambiance-custom");
  const format = getFieldValue("img-format-select", "img-format-custom");
  const detail = getFieldValue("img-detail-select", "img-detail-custom");
  const inspiration = getFieldValue("img-inspiration-select", "img-inspiration-custom");
  const sujet = document.getElementById("img-sujet-custom") ? document.getElementById("img-sujet-custom").value.trim() : "";
  const composition = getFieldValue("img-composition-select", "img-composition-custom");
  const contexte = getFieldValue("img-contexte-select", "img-contexte-custom");
  const elements = document.getElementById("img-elements-custom") ? document.getElementById("img-elements-custom").value.trim() : "";
  const qualite = getFieldValue("img-qualite-select", "img-qualite-custom");
  const detailDemande = document.getElementById("detail-demande-image") ? document.getElementById("detail-demande-image").value.trim() : "";

  let basePrompt = `<demande_generation_image>
  <specifications_visuelles>
    ${type ? `<type>${type}</type>` : ""}
    ${style ? `<style_artistique>${style}</style_artistique>` : ""}
    ${couleurs ? `<palette_couleurs>${couleurs}</palette_couleurs>` : ""}
    ${eclairage ? `<eclairage>${eclairage}</eclairage>` : ""}
  ...
  <composition>
    ${sujet ? `<sujet_principal>${sujet}</sujet_principal>` : ""}
    ${composition ? `<cadrage>${composition}</cadrage>` : ""}
    ${contexte ? `<environnement>${contexte}</environnement>` : ""}
    ${elements ? `<elements_specifiques>${elements}</elements_specifiques>` : ""}
  </composition>
  ...
  ${detailDemande ? `<description_complete>\n${detailDemande}\n  </description_complete>` : ""}
</demande_generation_image>`;

  const vsInstructions = `<mission_verbalized_sampling_image>

<persona_expert>
Tu es un expert en direction artistique et génération d'images IA. Tu maîtrises le Verbalized Sampling pour explorer des interprétations visuelles créatives et variées.
</persona_expert>

<methodologie_vs_visuelle>
Le Verbalized Sampling appliqué aux images permet d'explorer différentes interprétations visuelles d'un même concept. Chaque variation offre une approche esthétique unique.

<process>
1. ANALYSE des éléments visuels clés et des axes créatifs possibles
2. GÉNÉRATION de 5 variations visuelles distinctes :
   • Variation 1 : RÉALISME PHOTOGRAPHIQUE - Approche documentaire et authentique
   • Variation 2 : ARTISTIQUE STYLISÉ - Interprétation créative et expressive
   • Variation 3 : MINIMALISTE ÉPURÉ - Focus sur l'essentiel avec simplicité
   • Variation 4 : DRAMATIQUE CINÉMATIQUE - Composition spectaculaire et impactante
   • Variation 5 : CONCEPTUEL INNOVANT - Vision originale et inattendue
3. RECOMMANDATION de la variation la plus adaptée
</process>
</methodologie_vs_visuelle>
...
## 🎨 VARIATION VISUELLE [N]/5 : [NOM STYLE]
### 🖼️ Description prompt image :
\`\`\`
[Description complète pour générateur d'images IA]
\`\`\`

### 🎭 Approche visuelle :
[Explication de la direction artistique choisie]
...
[Pourquoi cette interprétation est la plus pertinente]
</format_reponse>
...
</mission_verbalized_sampling_image>`;

  document.getElementById("result-image").textContent = vsInstructions;
  openInLLM(vsInstructions);
}
...
// Génération du prompt VS pour les voyages
function generateVSVoyagePrompt() {
  const destination = document.getElementById("voyage-destination-custom") ? document.getElementById("voyage-destination-custom").value.trim() : "";
  const objectif = getFieldValue("voyage-objectif-select", "voyage-objectif-custom");
  const type = getFieldValue("voyage-type-select", "voyage-type-custom");
  const voyageurs = getFieldValue("voyage-voyageurs-select", "voyage-voyageurs-custom");
  const duree = getFieldValue("voyage-duree-select", "voyage-duree-custom");
  const budget = getFieldValue("voyage-budget-select", "voyage-budget-custom");
  const saison = getFieldValue("voyage-saison-select", "voyage-saison-custom");
  const contraintes = getFieldValue("voyage-contraintes-select", "voyage-contraintes-custom");
  const style = getFieldValue("voyage-style-select", "voyage-style-custom");
  const references = getFieldValue("voyage-references-select", "voyage-references-custom");
  const activites = getFieldValue("voyage-activites-select", "voyage-activites-custom");
  const hebergement = getFieldValue("voyage-hebergement-select", "voyage-hebergement-custom");
  const transport = getFieldValue("voyage-transport-select", "voyage-transport-custom");
  const interets = getFieldValue("voyage-interets-select", "voyage-interets-custom");
  const detailDemande = document.getElementById("detail-demande-voyage") ? document.getElementById("detail-demande-voyage").value.trim() : "";
  ...
  <preferences>
    ${style ? `<style_voyage>${style}</style_voyage>` : ""}
    ${references ? `<inspirations>${references}</inspirations>` : ""}
    ${activites ? `<activites_souhaitees>${activites}</activites_souhaitees>` : ""}
    ${interets ? `<centres_interet>${interets}</centres_interet>` : ""}
  </preferences>
  ...
  <logistique>
    ${hebergement ? `<type_hebergement>${hebergement}</type_hebergement>` : ""}
    ${transport ? `<mode_transport>${transport}</mode_transport>` : ""}
    ${contraintes ? `<contraintes_particulieres>${contraintes}</contraintes_particulieres>` : ""}
  </logistique>
  
  ${detailDemande ? `<informations_complementaires>\n${detailDemande}\n  </informations_complementaires>` : ""}
</demande_planification_voyage>`;

  const vsInstructions = `<mission_verbalized_sampling_voyage>

<persona_expert>
Tu es un expert en planification de voyages avec une connaissance approfondie des destinations mondiales. Tu utilises le Verbalized Sampling pour proposer des itinéraires créatifs et variés.
</persona_expert>

<methodologie_vs_voyage>
Le Verbalized Sampling appliqué à la planification de voyages génère plusieurs itinéraires distincts, chacun offrant une expérience unique adaptée aux mêmes critères de base.

<process>
1. ANALYSE du profil voyageur et des attentes
2. GÉNÉRATION de 5 itinéraires différents :
   • Itinéraire 1 : CLASSIQUE COMPLET - Les incontournables optimisés
   • Itinéraire 2 : HORS SENTIERS BATTUS - Découvertes authentiques
   • Itinéraire 3 : IMMERSION CULTURELLE - Focus expériences locales
   • Itinéraire 4 : AVENTURE & NATURE - Activités et sensations
   • Itinéraire 5 : CRÉATIF HYBRIDE - Mix original d'expériences
3. RECOMMANDATION de l'itinéraire le plus adapté
</process>
</methodologie_vs_voyage>
...
## ✈️ ITINÉRAIRE [N]/5 : [NOM THÉMATIQUE]
### 🗺️ Plan de voyage :
\`\`\`
...
[Approche et expérience proposée]
...
### 🎯 Meilleur itinéraire : [N]
### 💬 Pourquoi ce choix :
[Adéquation avec le profil et les attentes]

### 🔀 Variante hybride :
[Suggestion de combinaison si pertinent]
</format_reponse>
...
</mission_verbalized_sampling_voyage>`;

  document.getElementById("result-voyage").textContent = vsInstructions;
  openInLLM(vsInstructions);
}
...
// Génération du prompt VS pour le code
function generateVSCodePrompt() {
  const objectif = getFieldValue("objectif-select-code", "objectif-text-code");
  const role = getFieldValue("role-select-code", "role-text-code");
  const langage = getFieldValue("langage-select-code", "langage-text-code");
  const projet = getFieldValue("projet-select-code", "projet-text-code");
  const contraintes = getFieldValue("contraintes-select-code", "contraintes-text-code");
  const fichiers = getFieldValue("fichiers-select-code", "fichiers-text-code");
  const format = getFieldValue("format-select-code", "format-text-code");
  const niveau = getFieldValue("niveau-select-code", "niveau-text-code");
  const detailDemande = document.getElementById("detail-demande-code") ? document.getElementById("detail-demande-code").value.trim() : "";

  let basePrompt = `<demande_generation_code>
  <contexte_dev>
    ${objectif ? `<objectif>${objectif}</objectif>` : ""}
    ${role ? `<role_ia>${role}</role_ia>` : ""}
    ${langage ? `<langage_programmation>${langage}</langage_programmation>` : ""}
    ${projet ? `<type_projet>${projet}</type_projet>` : ""}
  </contexte_dev>
  
  <specifications>
    ${contraintes ? `<contraintes>${contraintes}</contraintes>` : ""}
    ${fichiers ? `<contexte_fichiers>${fichiers}</contexte_fichiers>` : ""}
    ${format ? `<format_sortie>${format}</format_sortie>` : ""}
    ${niveau ? `<niveau_detail>${niveau}</niveau_detail>` : ""}
  </specifications>
  
  ${detailDemande ? `<details_techniques>\n${detailDemande}\n  </details_techniques>` : ""}
</demande_generation_code>`;

  const vsInstructions = `<mission_verbalized_sampling_code>

<persona_expert>
Tu es un architecte logiciel senior avec une expertise multi-langage. Tu utilises le Verbalized Sampling pour explorer différentes approches techniques et patterns de code.
</persona_expert>

<methodologie_vs_code>
Le Verbalized Sampling appliqué au code génère plusieurs implémentations différentes, chacune avec ses avantages architecturaux et techniques.

<process>
1. ANALYSE des exigences techniques et contraintes
2. GÉNÉRATION de 5 approches de code :
   • Approche 1 : MAINTENABILITÉ - Code propre, documenté, évolutif
   • Approche 2 : PERFORMANCE - Optimisations et efficacité maximale
   • Approche 3 : SIMPLICITÉ - Solution minimaliste et directe
   • Approche 4 : ROBUSTESSE - Gestion d'erreurs et edge cases
   • Approche 5 : MODERNE - Patterns et syntaxe récents
3. RECOMMANDATION de l'approche la plus adaptée
</process>
</methodologie_vs_code>
...
### 🎯 Meilleure approche : [N]
### 💬 Justification technique :
[Pourquoi cette implémentation est optimale pour le contexte]

### 🔧 Compromis suggéré :
[Combinaison d'éléments de différentes approches si pertinent]
</format_reponse>
...
</mission_verbalized_sampling_code>`;

  document.getElementById("result-code").textContent = vsInstructions;
  openInLLM(vsInstructions);
}
...
// Génération du prompt VS pour les applications
function generateVSAppPrompt() {
  const type = getFieldValue("type-select-app", "type-text-app");
  const stack = getFieldValue("stack-select-app", "stack-text-app");
  const platform = getFieldValue("platform-select-app", "platform-text-app");
  const fonction = getFieldValue("fonction-select-app", "fonction-text-app");
  ...
  const tests = getFieldValue("tests-select-app", "tests-text-app");
  const livrables = getFieldValue("livrables-select-app", "livrables-text-app");
  const contraintes = getFieldValue("contraintes-select-app", "contraintes-text-app");
  const publicCible = getFieldValue("public-select-app", "public-text-app");
  const detailDemande = document.getElementById("detail-demande-app") ? document.getElementById("detail-demande-app").value.trim() : "";

  let basePrompt = `<demande_application>
  <specifications_base>
    ${type ? `<type>${type}</type>` : ""}
    ${stack ? `<stack_technique>${stack}</stack_technique>` : ""}
    ${platform ? `<plateforme>${platform}</plateforme>` : ""}
    ${fonction ? `<fonctionnalites>${fonction}</fonctionnalites>` : ""}
  </specifications_base>
  
  <architecture>
    ${archi ? `<pattern_architectural>${archi}</pattern_architectural>` : ""}
    ${data ? `<base_donnees>${data}</base_donnees>` : ""}
    ${auth ? `<authentification>${auth}</authentification>` : ""}
  </architecture>
  
  <interface_utilisateur>
    ${design ? `<framework_ui>${design}</framework_ui>` : ""}
    ${a11y ? `<accessibilite>${a11y}</accessibilite>` : ""}
    ${features ? `<features_additionnelles>${features}</features_additionnelles>` : ""}
  </interface_utilisateur>
  
  <qualite>
    ${perf ? `<optimisations>${perf}</optimisations>` : ""}
    ${tests ? `<tests>${tests}</tests>` : ""}
    ${livrables ? `<livrables>${livrables}</livrables>` : ""}
    ${contraintes ? `<contraintes>${contraintes}</contraintes>` : ""}
    ${publicCible ? `<public_cible>${publicCible}</public_cible>` : ""}
  ...
  ${detailDemande ? `<description_detaillee>\n${detailDemande}\n  </description_detaillee>` : ""}
</demande_application>`;

  const vsInstructions = `<mission_verbalized_sampling_application>

<persona_expert>
Tu es un architecte d'applications full-stack avec une vision produit. Tu utilises le Verbalized Sampling pour explorer différentes architectures et approches de développement d'applications.
</persona_expert>

<methodologie_vs_application>
Le Verbalized Sampling appliqué aux applications génère plusieurs architectures et implémentations possibles, chacune optimisée pour des priorités différentes.

<process>
1. ANALYSE des besoins fonctionnels et contraintes techniques
2. GÉNÉRATION de 5 architectures d'application :
   • Architecture 1 : MVP RAPIDE - Mise en production accélérée
   • Architecture 2 : SCALABILITÉ - Conçue pour la croissance
   • Architecture 3 : UX PREMIUM - Expérience utilisateur optimale
   • Architecture 4 : SÉCURITÉ RENFORCÉE - Protection maximale
   • Architecture 5 : INNOVATION TECHNIQUE - Stack moderne et performante
3. RECOMMANDATION de l'architecture la plus adaptée
</process>
</methodologie_vs_application>
...
[Approche et priorités de cette architecture]
...
### ⚠️ Considérations :
[Points d'attention ou trade-offs]
...
### 🎯 Meilleure approche : [N]
### 💬 Justification :
[Pourquoi cette architecture est optimale pour le contexte et les objectifs]

### 🔀 Architecture hybride :
[Suggestion de combinaison d'éléments de différentes architectures si pertinent]

### 📋 Prochaines étapes :
[Plan d'action pour démarrer le développement]
</format_reponse>
...
</mission_verbalized_sampling_application>`;

  document.getElementById("result-app").textContent = vsInstructions;
  openInLLM(vsInstructions);
}
